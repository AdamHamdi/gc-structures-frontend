...
function EditButton({ onClick }: { onClick: () => void }) {
  return (
    <button className="btn-danger-2 btn-modif rounded-50 px-3 flex items-center" onClick={onClick}>
...
function EditIconButton({ onClick }: { onClick: () => void }) {
  return (
    <button className="edit-icon-btn" onClick={onClick} title="Modifier">
      <FaPencilAlt size={14} />
    </button>
  );
}
...
function EditableSection({
  sectionTitle, content, onSave, fields,
}: {
  sectionTitle: string;
  ...
  useEffect(() => { setTempData(content); }, [content]);
  ...
  const updateField = (key: string, value: string) => setTempData({ ...tempData, [key]: value });
            ...
            <div className="drop-down-cont">
              <div className="drop-down p-5" style={{ minWidth: '400px' }}>
                ...
                <div className="flex justify-end items-center mt-3 gap-2">
                  <button className="btn-danger height-35" onClick={() => { setDropdownVisible(false); setTempData(content); }}>Annuler</button>
                  <button className="btn-blue height-35" onClick={handleSave}>{isLoading ? "En cours..." : "Sauvegarder"}</button>
                </div>
              </div>
            </div>
          )}
...
function EditableItemsSection({
  sectionTitle, content, onSave, itemLabel,
}: {
  sectionTitle: string;
  content: SectionContent;
  onSave: (content: SectionContent) => Promise<void>;
  ...
  useEffect(() => { setTempTitle(content?.title || ''); setTempItems(content?.items || []); }, [content]);

  const handleSave = async () => {
    setIsLoading(true);
    try {
      await onSave({ ...content, title: tempTitle, items: tempItems });
      setDropdownVisible(false);
    } catch (error) {
      console.error("Erreur:", error);
    } finally {
        ...
        <div className="flex flex-wrap gap-3">
          {tempItems.map((item, index) => (
            <div key={index} className="bg-white p-4 rounded-lg flex-1 min-w-[200px] relative">
              <div className="absolute top-2 right-2"><EditIconButton onClick={() => setDropdownVisible(true)} /></div>
          ...
          {isDropdownVisible && (
            <div className="drop-down-cont">
              <div className="drop-down p-5" style={{ minWidth: '500px', maxHeight: '70vh', overflowY: 'auto' }}>
                ...
                <textarea value={tempTitle} onChange={(e) => setTempTitle(e.target.value)} rows={1} />
                {tempItems.map((item, index) => (
                  <div key={index} className="mt-4 p-3 bg-gray-50 rounded relative">
                    <button className="absolute top-2 right-2 text-red-500 hover:text-red-700" onClick={() => setTempItems(tempItems.filter((_, i) => i !== index))}>×</button>
                    <label className="fz-16 text-bold">{itemLabel} {index + 1}</label>
                    <input type="text" value={item.title} onChange={(e) => updateItem(index, 'title', e.target.value)} className="w-full p-2 border rounded mt-2" placeholder="Titre" />
                    <textarea value={item.description} onChange={(e) => updateItem(index, 'description', e.target.value)} rows={2} className="mt-2" placeholder="Description" />
                  </div>
                ))}
                <button className="mt-3 text-blue-600 hover:text-blue-800 text-sm" onClick={() => setTempItems([...tempItems, { title: '', description: '' }])}>+ Ajouter un {itemLabel.toLowerCase()}</button>
                <div className="flex justify-end items-center mt-3 gap-2">
                  <button className="btn-danger height-35" onClick={() => setDropdownVisible(false)}>Annuler</button>
                  <button className="btn-blue height-35" onClick={handleSave}>{isLoading ? "En cours..." : "Sauvegarder"}</button>
                </div>
              </div>
            </div>
...
export default function ServiceAdminPage({ pageName, pageTitle, sections }: ServiceAdminPageProps) {
  ...
  const fetchPageContent = async () => {
    setLoading(true);
    try {
      const data = await getCmsPage(pageName);
      setPageContent(data);
    } catch (error) {
      console.error("Erreur lors du chargement du contenu:", error);
      const defaultSections: { [key: string]: SectionContent } = {};
      sections.forEach((s) => { defaultSections[s.id] = { title: '', description: '' }; });
      setPageContent({ PageName: pageName, sections: defaultSections });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveSection = async (sectionId: string, newContent: SectionContent) => {
    try {
      ...
      await updateCmsSection(token, pageName, sectionId, newContent);
      setPageContent((prev) => {
        if (!prev) return prev;
        return { ...prev, sections: { ...prev.sections, [sectionId]: newContent } };
      });
  ...
  return (
    <div className="services-items">
      <div className="services-items-content">
        <h1 className="text-blue text-bold fz-30 mb-5">Gestion du contenu - {pageTitle}</h1>
